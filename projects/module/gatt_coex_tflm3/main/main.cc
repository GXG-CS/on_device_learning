...
#define GATTS_ADV_NAME              "ESP_GATT_COEX"
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        // Log the connection ID and length of received data
        ESP_LOGI(COEX_TAG, "Connection ID: %d", p_data->notify.conn_id);
        ESP_LOGI(COEX_TAG, "Remote Device Address:");
        esp_log_buffer_hex(COEX_TAG, p_data->notify.remote_bda, sizeof(esp_bd_addr_t));

        // Log the attribute handle
        ESP_LOGI(COEX_TAG, "Attribute Handle: %d", p_data->notify.handle);
        ESP_LOGI(COEX_TAG, "Received Data Length: %d", p_data->notify.value_len);

        // Store received data for running Layer 2
        memcpy(received_data, p_data->notify.value, p_data->notify.value_len);
        
        // Set flag to start inference for Layer 2
        new_data_available = true;
        ESP_LOGI(COEX_TAG, "Data received and stored. new_data_available flag set to true.");

        // Log received data in hex format for debugging
        ESP_LOGI(COEX_TAG, "Received Data in Hex:");
        esp_log_buffer_hex(COEX_TAG, p_data->notify.value, p_data->notify.value_len);

        ESP_LOGI(COEX_TAG, "Data stored successfully. Ready to start Layer 2 inference.");
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_REG_EVT:
        ...
        set_dev_name_ret = esp_ble_gap_set_device_name(GATTS_ADV_NAME);
        if (set_dev_name_ret) {
            ...
            if (gatts_profile_tab[GATTS_PROFILE_A_APP_ID].descr_handle == param->write.handle && param->write.len == 2) {
                ...
                if (descr_value == NOTIFY_ENABLE) {
                    if (a_property & ESP_GATT_CHAR_PROP_BIT_NOTIFY) {
                        ESP_LOGI(COEX_TAG, "notify enable a");
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        add_char_ret = esp_ble_gatts_add_char(gatts_profile_tab[GATTS_PROFILE_A_APP_ID].service_handle, &gatts_profile_tab[GATTS_PROFILE_A_APP_ID].char_uuid,
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
            ...
            if (gatts_profile_tab[GATTS_PROFILE_B_APP_ID].descr_handle == param->write.handle && param->write.len == 2) {
                ...
                if (descr_value == NOTIFY_ENABLE) {
                    if (b_property & ESP_GATT_CHAR_PROP_BIT_NOTIFY) {
                        ESP_LOGI(COEX_TAG, "notify enable b");
                        uint8_t notify_data[15];
                        // for (int i = 0; i < sizeof(notify_data); ++ i) {
                        //     notify_data[i] = i % 0xff;
                        // }
    ...
    case ESP_GATTS_CREATE_EVT:
        ...
        add_char_ret = esp_ble_gatts_add_char(gatts_profile_tab[GATTS_PROFILE_B_APP_ID].service_handle, &gatts_profile_tab[GATTS_PROFILE_B_APP_ID].char_uuid,
                                                        ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
                                                        b_property,
                                                        NULL, NULL);
        if (add_char_ret) {
...
// The setup function to initialize the TFLite model, interpreter, and tensors.
void setup() {
    // Set up logging
    ESP_LOGI(COEX_TAG, "Initializing TensorFlow Lite Micro.");

    // Load model from memory (linked binary)
    ...
    // Define operations resolver to specify which operations the interpreter will use
    static tflite::MicroMutableOpResolver<6> micro_op_resolver;  // Adjust the number to the operations required
    micro_op_resolver.AddFullyConnected();
    micro_op_resolver.AddRelu();
    micro_op_resolver.AddSoftmax();
    micro_op_resolver.AddConv2D();
    micro_op_resolver.AddMaxPool2D();
    micro_op_resolver.AddTanh();  // Assuming the second layer uses a Tanh activation function
    ...
    if (allocate_status != kTfLiteOk) {
        ESP_LOGE(COEX_TAG, "AllocateTensors() failed");
        return;
    }
    ...
    ESP_LOGI(COEX_TAG, "Setup complete. Model initialized and interpreter ready.");
}



extern "C" void app_main(void) {
    // ESP_LOGI(COEX_TAG, "Starting initialization.");
    // setup();
    esp_err_t ret;
    ...
    ESP_LOGI(COEX_TAG, "Waiting for notifications from Layer 1.");
    int log_counter = 0;
    while (true) {
        // ESP_LOGI(COEX_TAG, "Returning to main loop.");
        
        if (new_data_available) {
            ESP_LOGI(COEX_TAG, "Starting inference with received data.");

            // Assuming received_data contains the data for Layer 2
            float input_layer2[2];  // Adjust size based on actual layer input requirements
            memcpy(input_layer2, received_data, sizeof(input_layer2));  // Copy only the required size

            // Log input data for debugging
            ESP_LOGI(COEX_TAG, "Received data for Layer 2:");
            ESP_LOGI(COEX_TAG, "input_layer2[0]: %f", input_layer2[0]);
            ESP_LOGI(COEX_TAG, "input_layer2[1]: %f", input_layer2[1]);

            // Ensure input tensor dimensions are correct
            ESP_LOGI(COEX_TAG, "Input tensor dimensions: %d", input->dims->data[1]);
            for (int i = 0; i < input->dims->data[1]; ++i) {
                input->data.f[i] = input_layer2[i];
            }

            // Log input tensor for debugging
            ESP_LOGI(COEX_TAG, "Input tensor before inference:");
            for (int i = 0; i < input->dims->data[1]; ++i) {
                ESP_LOGI(COEX_TAG, "input_tensor[%d]: %f", i, input->data.f[i]);
            }

            // Run inference for Layer 2
            TfLiteStatus invoke_status = interpreter->Invoke();
            if (invoke_status == kTfLiteOk) {
                ESP_LOGI(COEX_TAG, "Layer 2 Inference complete. Output:");


                // Prepare the output to send to the client device
                float output_data[2];  // Assuming 2 outputs for Layer 2
                for (int i = 0; i < output->dims->data[1]; ++i) {
                    output_data[i] = output->data.f[i];
                    ESP_LOGI(COEX_TAG, "output[%d]: %f", i, output_data[i]);
                }

                esp_err_t err = ESP_OK;
                if (notify_enabled) {
                    // Convert the float output data to bytes
                    uint8_t output_bytes[sizeof(output_data)];
                    memcpy(output_bytes, output_data, sizeof(output_data));

                    // Send the output via BLE notification
                    esp_err_t err = esp_ble_gatts_send_indicate(
                        gatts_profile_tab[GATTS_PROFILE_A_APP_ID].gatts_if, 
                ...
                if (err != ESP_OK) {
                    ESP_LOGE(COEX_TAG, "Failed to send notification, error code = %x", err);
                } else {
                    ESP_LOGI(COEX_TAG, "Output data sent to client successfully.");
                }

            } else {
                ESP_LOGE(COEX_TAG, "Inference failed for Layer 2");
            }

            

            // Clear the flag after inference
        ...
        if (log_counter++ % 10 == 0) {  // Log every 10 iterations
            ESP_LOGI(COEX_TAG, "Still in main loop, waiting for new data.");
        }
