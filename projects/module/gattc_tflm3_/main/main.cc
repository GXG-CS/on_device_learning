...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        // Free the previous buffer if needed
        if (received_data) {
            free(received_data);
        }

        // Allocate new memory directly based on the value length
        received_data = (uint8_t*)malloc(p_data->notify.value_len);
        if (received_data == NULL) {
            ESP_LOGE(GATTC_TAG, "Failed to allocate memory for received data.");
            break;
        }
...
extern "C" void app_main(void) {
    ...
    int mock_input_data[4] = {1,2,3,4};  // Replace with actual size and values as per your model
