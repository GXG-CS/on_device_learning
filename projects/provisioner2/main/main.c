...
static void periodic_timer_callback(void* arg) {
    ...
    // Assuming the first provisioned node will be controlled
    node = &nodes[0];

    if (node->unicast == ESP_BLE_MESH_ADDR_UNASSIGNED) {
        ESP_LOGE(TAG, "Node not assigned");
        return;
    }

    example_ble_mesh_set_msg_common(&common, node, onoff_client.model, ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET);
    set_state.onoff_set.op_en = false;
    set_state.onoff_set.onoff = !node->onoff;  // Toggle the OnOff state
    ...
    ESP_LOGI(TAG, "Sent Generic OnOff Set command, onoff: %d", set_state.onoff_set.onoff);
}
...
static void display_provisioned_nodes_task(void *arg) {
    while (1) {
        ESP_LOGI(TAG, "Provisioned devices:");
        for (int i = 0; i < ARRAY_SIZE(nodes); i++) {
            if (nodes[i].unicast != ESP_BLE_MESH_ADDR_UNASSIGNED) {
                ESP_LOGI(TAG, "Node %d: UUID: %s, Unicast Address: 0x%04x, Element Count: %d, OnOff State: %d",
                         i, bt_hex(nodes[i].uuid, 16), nodes[i].unicast, nodes[i].elem_num, nodes[i].onoff);
            }
        }
...
void app_main(void)
{
    ...
    xTaskCreate(display_provisioned_nodes_task, "display_provisioned_nodes_task", 4096, NULL, 5, NULL);
    ...
    ESP_ERROR_CHECK(esp_timer_start_periodic(periodic_timer, 10000000));  // 10 seconds
