...
# Define the corrected partition heights and row indices
partition_indices = [
    {"partition": 1, "start_row": 1, "end_row": 51, "height": 51},
    {"partition": 2, "start_row": 45, "end_row": 95, "height": 51},
    {"partition": 3, "start_row": 89, "end_row": 139, "height": 51},
    {"partition": 4, "start_row": 133, "end_row": 183, "height": 51},
    {"partition": 5, "start_row": 177, "end_row": 227, "height": 51}
]

# Create new models for each partition using the original weights and quantize them
for partition_info in partition_indices:
    ...
    partition_height = partition_info["height"]

    print(f"Creating and quantizing model for Partition {partition_id} with rows {start_row} to {end_row} (Height: {partition_height})")
    ...
    # Convert the partitioned model to TFLite format with INT8 quantization
    converter = tf.lite.TFLiteConverter.from_keras_model(partition_model)
    # Enable full integer quantization
    converter.optimizations = [tf.lite.Optimize.DEFAULT]

    # Provide a representative dataset that matches the partition height
    def representative_dataset():
        for _ in range(100):
            yield [np.random.rand(1, partition_height, 227, 3).astype(np.float32)]
    ...
    # Save the INT8 quantized model as a .tflite file
    filename = f"alexnet_conv1_partition_{partition_id}_int8.tflite"
    with open(filename, "wb") as f:
        f.write(tflite_model_int8)
        print(f"Saved INT8 TFLite model for Partition {partition_id}: {filename}")

print("All partitioned models created and quantized successfully!")
